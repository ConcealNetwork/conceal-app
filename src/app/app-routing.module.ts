...
const routes: Routes = [
	...
	// Bridge module intentionally disabled from routing to exclude from build
	// { path: 'bridge', loadChildren: () => import('./modules/bridge/bridge.module').then(m => m.BridgeModule), data: { title: "Conceal Bridge" } },
	// { path: 'authenticator', loadChildren: () => import('./modules/authenticator/authenticator.module').then(m => m.AuthenticatorModule), data: { title: "Conceal Authenticator" } },
